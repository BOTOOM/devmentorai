...
export async function imagesRoutes(fastify: FastifyInstance) {
  ...
  }>('/upload/:sessionId/:messageId', async (request, reply) => {
    try {
      ...
      console.log(
        `[ImagesRoute] Pre-uploading ${body.images.length} images for ${sessionId}/${messageId}`
      );
  ...
  fastify.get<{ Params: ImageParams }>(
    ...
    async (request: FastifyRequest<{ Params: ImageParams }>, reply: FastifyReply) => {
      ...
      if (thumbMatch) {
        const imageIndex = Number.parseInt(thumbMatch[1], 10);
