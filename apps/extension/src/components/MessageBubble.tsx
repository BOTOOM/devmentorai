import type { ImageAttachment, Message } from '@devmentorai/shared';
import {
  AlertTriangle,
  Bot,
  Check,
...
export function MessageBubble({ message, onReplaceText }: Readonly<MessageBubbleProps>) {
  ...
  const isReplaceableAction =
    !isUser && (action === 'translate' || action === 'rewrite' || action === 'fix_grammar');
  let bubbleClassName = 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white rounded-tl-md';

  if (isUser) {
    bubbleClassName = 'bg-primary-600 text-white rounded-tr-md';
  } else if (isSystem) {
    bubbleClassName =
